...
class Snowflake {
  ...
  public draw(ctx: CanvasRenderingContext2D): void {
    // Set the properties for the hair strand
    ctx.strokeStyle = this.config.color; // Color of the hair strand
    ctx.lineWidth = this.params.radius; // Thickness of the hair strand

    // Calculate random variations for control points
    const cp1x = this.params.x - this.params.radius * 2;

    const cp1y = this.params.y + this.params.speed * 2;
    const cp2x = this.params.x + this.params.radius * 2;
    const cp2y = this.params.y + this.params.speed * 8;

    // Draw the hair strand with curly and varied movement
    ctx.beginPath();
    ctx.moveTo(this.params.x, this.params.y); // Starting point of the hair strand
    ctx.bezierCurveTo(
      cp1x,
      cp1y,
      cp2x,
      cp2y,
      this.params.x + this.params.speed * 5,
      this.params.y + this.params.speed * 10
    ); // Ending point of the hair strand
    ctx.stroke();
  }
}
